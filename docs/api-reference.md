...
| POST | `/auth/change-password` | any authenticated | Body: `{ currentPassword, newPassword }` (`newPassword` min 12 chars, same floor as account creation). Verifies `currentPassword` against the stored hash (`401` if wrong), then rehashes and stamps `User.passwordChangedAt = now()`. Every token issued before that stamp — the one used to make this call included — is rejected by `JwtStrategy` on its very next use; the response returns a freshly-signed `access_token` so the caller's own session keeps working. No frontend UI calls this yet — API-only this pass. See `docs/security-architecture.md`'s "Token revocation". |
...
| GET | `/initiatives?organisationId=&status=&sortBy=&search=&page=&pageSize=` | any authenticated, **except EXECUTIVE_VIEWER** | Organisation-scoped list. **Paginated** — `PaginatedResponse<RemediationInitiative>`, default `pageSize` 20, capped at 100. `search` is a case-insensitive substring match against `title` (Postgres `contains`/`insensitive`). The risk-detail page's initiative picker uses `search` + real page navigation (10 per page) rather than requesting the largest allowed page and hoping it covers every initiative, so tenants with more than 100 initiatives can still find and link any of them. |
